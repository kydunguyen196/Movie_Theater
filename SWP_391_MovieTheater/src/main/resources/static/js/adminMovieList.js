...
function showMovieDetails(element) {
    let movieId = $(element).data('movie-id');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#inputPromotion').val(movie.promotion);
            $('#inputMovieProductionCompany').val(movie.movieProductionCompany);
            $('#inputDuration').val(movie.duration);
            $('#inputFromDate').val(movie.fromDate).slice(0, 10);
            $('#inputToDate').val(movie.toDate).slice(0, 10);
            $('#inputVnName').val(movie.movieNameVietnamese);
            $('#inputEngName').val(movie.movieNameEnglish);
                ...
                Promise.all([
                    ...
                    .then(function ([srcSmallImage, srcLargeImage]) {
                        ...
                        var movie = {
                            ...
                            movieNameEnglish: $('#inputEngName').val().trim(),
                            movieNameVietnamese: $('#inputVnName').val().trim(),
                            fromDate: $('#inputFromDate').val().trim(),
                            toDate: $('#inputToDate').val().trim(),
                            duration: $('#inputDuration').val().trim(),
                            actor: $('#inputActor').val().trim(),
                            movieProductionCompany: $('#inputMovieProductionCompany').val().trim(),
                            promotion: $('#inputPromotion').val().trim(),
...
function addNewMovie() {
    $('#add-new-movie').on('click', async function () {
        ...
        try {
            validateFormInputs();
            const fromDate = $('#inputFromDate').val().trim();
            const toDate = $('#inputToDate').val().trim();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(movieList, function (index, value) {
                var html = `
                    <div class="col-12 col-sm-6 col-md-4 col-lg-3 movie-display" onclick="showMovieDetails(this)" data-movie-id="` + value.movieId + `">
                    <div class="card bg bg-dark fw-danger my-2">
                        ...
                        <div class="card-body ">
                            <h5 class="card-title product-title">
                                <a class="text-lgiht-emphasis text-decoration-none" style="color: white">
                                    <span class="movie-name">` + value.movieNameEnglish + `</span>
                                </a>
                            </h5>
                            <p class="card-text text-white-50">` + value.content.substring(0, 160) + '...' + `</p>
                        </div>
                    </div>
                </div>
            `
...
function validateDates(fromDate, toDate) {
    ...
    if (from < current || to < current) throw new Error("Dates must not be in the past.");
}
